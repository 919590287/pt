...
| 统一筛选「是否显示扩样结果」 | **砍掉**，统一显示模型原始量 | 与运行监测/客流分析口径一致：模型中有多少展示多少，不对绝对量做扩大、缩小或抽样 |
...
| 数量口径 | 已加载模型的原始记录 | 不根据 `scale` 扩大、缩小或抽样 |
...
事件记录字段：`{personIndex, dir, tAlight, tBoard, busLine, busRoute, busStop, busOriginStop, busDestinationStop, metroLine, metroStop, hub}`，其中 `dir ∈ {bus→metro, metro→bus}`；`busOriginStop` / `busDestinationStop` 分别是整段公交乘车的起点与终点，用于两个换乘方向统一还原完整公交段；`personIndex` 为模型内匿名自增索引（同一 person 恒定同一索引，不下发原始 personId），用于任意筛选切面下的去重人数计算。
...
- 绝对量（换乘人次、人数）直接展示已加载模型的原始数量；`desc.json.scale` 仅作旧数据兼容元信息，不参与任何计算。
...
- 事件表按列式二进制编码（见 §11.2），25 B/事件，5 万事件约 1.25MB，走既有 `.bin` + ETag 通道，`modelDataCache` 记忆化。
...
- **核心指标卡**（6 个）：换乘人次（模型原始量）、公交→地铁人次、地铁→公交人次、换乘人数（去重）、平均换乘时间、P90 换乘时间。
- **图表**：
...
- **接驳率分母（固定口径，指标展示名必须带分母范围，不让用户猜）**：
  ...
  - 分母由 `transfer-dict.json` 的 `busLines[].boardings/alightings` 提供（换乘缓存从 personTracks 单遍扫描自产，全线全日、模型原始口径——routePanel 只有上车人次没有下车人次，故不复用）。如后续需要"仅换乘枢纽处上下车"分母，另立新指标名，不得复用同名。
- **图表**：
...
- **核心指标卡**：平均/中位/P90 换乘时间、5/10/15min 内完成比例、长换乘人次（模型原始量）、长换乘比例。
- **图表**：
...
```jsonc
{
  ...
  "totals": {
    "events": 48213, "persons": 39102,   // 模型原始量；persons=全网去重（筛选态人数由 Worker 按 personIndex 重算）
...
| `busOriginStop` | u16 | 整段公交乘车起点的 `busStops` 字典索引 |
| `busDestinationStop` | u16 | 整段公交乘车终点的 `busStops` 字典索引；与起点共同绘制两个方向的完整公交段 |
| `metroLine` | u16 | 字典索引 |
| `metroStop` | u16 | 轨道侧实际上下车 facility 字典索引（枢纽详情"公交站→地铁站"连线与站台级分析用，与 §3.1 事件定义一致） |
| `hub` | u16 | 枢纽字典索引（含坐标） |

合计 **27 B/事件**（4+4+2+1+2+2+2+2+2+2+2+2），5 万事件 ≈ 1.35MB。原始 personId 不下发（隐私）；`personIndex` 仅模型内有效，跨模型不可比。
...
```jsonc
{
  ...
  "busLines":  [{ "lineId": "busgtfs_B301", "name": "B301",
                  "boardings": 1520, "alightings": 1498,   // 全线全日上/下车人次（模型原始口径），接驳率分母（§6.3）
...
8. **【砍掉项回顾】**：统计迭代筛选、数量口径开关、双地图/卷帘、闪烁动效、坡度/哑铃图、首期候车时间拆分——理由见 §1，如业务方坚持可在对应分期恢复评估。
...
**P0 地图链路补充（2026-07-20）**：缓存升级为 `transfer-v3` / bin version 2，增加 `busOriginStop`，事件记录由 23B 增至 25B；换乘枢纽口径统一为地铁站，地图复用客流分析的地铁线网样式，并在选中地铁站后绘制完整公交来向与换乘步行接驳链。

**换乘分析改版（2026-07-21）**：缓存升级为 `transfer-v4` / bin version 3，增加 `busDestinationStop`，事件记录由 25B 增至 27B；公交整段起终点及连线统一灰色，公交—地铁短连线合并双方向客流；线路分析改为地铁线路维度，换乘衔接子模块下线。
